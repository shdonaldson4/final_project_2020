...
# anova_desc_e <- anova_data_wide %>% 
#   group_by(country,gender) %>% 
#   summarize(mean_ex = mean(E), sd_ex = sd(E))
# anova_desc_e
...
# anova_data_long <- anova_data_wide%>%
#   pivot_longer(
#     cols = c(O, E),
...
# anova_data_long
# 
# anova_desc_plot <- anova_data_long %>% 
...
```{r anova descriptives table, results = "asis"}
...
```{r interaction_plot, fig.cap = "Interaction of Country and Gender on Openness"}
anova_interaction <- anova_data_wide %>% 
  select(country,gender, O) %>% 
  ggplot(aes(x = country, color = gender, group = gender, y = O)) +
  stat_summary(fun.y = mean, geom = "point") +
